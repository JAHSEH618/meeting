"""Aliyun OSS-backed read implementation of the artifact store protocol.

Design boundary (enforced by RAM credentials, not by this code):

* ``download`` / ``download_json`` / ``local_path`` — call OSS GetObject /
  HeadObject. The worker's RAM subaccount is provisioned with **only**
  ``oss:GetObject`` and ``oss:HeadObject``; any attempt to PUT or DELETE
  on real OSS will fail with ``AccessDenied`` server-side. That is the
...
* ``upload`` / ``upload_json`` / ``delete`` — delegated to an underlying
  ...
  URI still uses the ``oss://`` scheme so the contract pattern stays
  satisfied, but the bytes are NOT actually pushed to OSS. Today Java
...
class OssArtifactStore:
    """Read-via-OSS, write-via-local artifact store. See module docstring."""
    ...
    def __init__(
        ...
        if not endpoint or not endpoint.strip():
            raise ValueError("OssArtifactStore: endpoint is required")
        if not region or not region.strip():
            raise ValueError("OssArtifactStore: region is required")
        if not access_key_id or not access_key_secret:
            raise ValueError("OssArtifactStore: access_key_id and access_key_secret are required")
        self._endpoint = endpoint
        self._region = region
        self._auth = oss2.AuthV4(access_key_id, access_key_secret)
        self._local_writer = local_writer
        self._cache_dir = Path(cache_dir) if cache_dir else Path(tempfile.gettempdir()) / "ai-worker-oss"
        self._cache_dir.mkdir(parents=True, exist_ok=True)
        self._bucket_cache: dict[str, oss2.Bucket] = {}

    def _bucket(self, name: str) -> oss2.Bucket:
        if name not in self._bucket_cache:
            self._bucket_cache[name] = oss2.Bucket(
                self._auth,
                self._endpoint,
                name,
                region=self._region,
    ...
    async def upload(
        ...
        # the pod's emptyDir — RAM credentials deliberately lack PutObject.
    ...
    async def download(self, uri: str) -> bytes:
        bucket, key = _parse_oss_uri(uri)
        result = self._bucket(bucket).get_object(key)
        try:
            # oss2 stubs widen read() to include ``Literal['']``; in practice
            # the runtime always returns ``bytes`` (possibly ``b""``).
    ...
    async def delete(self, uri: str) -> None:
        ...
        # the local writer. Real OSS DeleteObject is performed by Java's
    ...
    def local_path(self, uri: str) -> Path:
        ...
        bucket_client = self._bucket(bucket)
        # HEAD first to know the expected size; if the object is missing
        # this raises NoSuchKey before we waste cycles streaming to disk.
        head = bucket_client.head_object(key)
        expected_size = getattr(head, "content_length", None)
        ...
        tmp = Path(tmp_str)
        try:
            bucket_client.get_object_to_file(key, str(tmp))
            actual_size = tmp.stat().st_size
            if expected_size is not None and actual_size != expected_size:
                raise OSError(
                    f"OssArtifactStore: short download for {uri}: "
...
def _parse_oss_uri(uri: str) -> tuple[str, str]:
    parsed = urlparse(uri)
    if parsed.scheme != "oss" or not parsed.netloc or not parsed.path:
        raise ValueError(f"OssArtifactStore expects oss://bucket/key, got: {uri}")
