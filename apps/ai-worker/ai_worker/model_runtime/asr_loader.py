"""ASR model loader with fallback strategy.

Provides a single entry point for loading ASR models with automatic fallback
from real models to fake implementations when weights are unavailable. This
abstraction allows production code to request the best available runtime
without knowing whether real model weights have been staged.

Usage:
    loader = ASRModelLoader()
    runtime = await loader.load()
    segments = await runtime.transcribe(audio_path, metadata, language)

The loader checks for model weights at the configured path. If found, it
returns a real Qwen3AsrRuntime. If not, it falls back to the fake/deterministic
...
class ASRModelLoader:
    """Load ASR models with automatic real→fake fallback.

    The loader inspects the configured model path and decides whether to use
    the real Qwen3-ASR runtime or fall back to the deterministic fake. This
    decision is made at load time, allowing the same application code to run
    with or without GPU hardware and model weights.
    """
    ...
    def __init__(
        ...
        Args:
            model_path: Path to Qwen3-ASR weights. If None, reads from
                AI_WORKER_QWEN3_ASR_MODELS_DIR environment variable.
                Defaults to /opt/models/qwen3-asr-1.7b/v2026.05.1 if unset.
        ...
        """
        default_path = "/opt/models/qwen3-asr-1.7b/v2026.05.1"
        env_path = os.getenv("AI_WORKER_QWEN3_ASR_MODELS_DIR", default_path)
        self.model_path = Path(model_path) if model_path else Path(env_path)
        self.device = device
        self.expected_checksum = expected_checksum
        self._runtime: Qwen3AsrRuntime | None = None
    ...
    async def load(self) -> Qwen3AsrRuntime:
        """Load the best available ASR runtime.

        Attempts to load the real Qwen3-ASR model if weights are available.
        Falls back to the deterministic fake implementation otherwise.

        Returns:
            A Qwen3AsrRuntime instance, configured for either real or fake mode.
    ...
    @property
    def runtime(self) -> Qwen3AsrRuntime | None:
