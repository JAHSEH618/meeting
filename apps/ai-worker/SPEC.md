...
所有任务共同必填 `taskId`、`taskType`、`tenantId`、`attemptNo`、`expectedInputVersion`、`options`、`traceId` 和 `pipelineSteps`。`EXPORT` 不属于 Python `ai-worker` 任务类型；导出消息使用 `packages/meeting-contracts/schemas/rabbitmq/export-job-message.schema.json`，由 Java `export-queue` consumer 处理。
