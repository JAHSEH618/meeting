...
9. `WORKER_PHASE_COMPLETED` 由 app 层 listener 异步消费；listener 根据 `taskType` 决定是否进入 `JAVA_LLM_RUNNING`，非 LLM worker 任务（`TEXT_EMBEDDING` / `RAG_REINDEX` / `SPEAKER_ENROLLMENT`）直接将 `phase` 置为 `TERMINAL` 并写入 task 终态。`EXPORT` 不进入 ai-worker phase 流程，由 Java `export-queue` consumer 单独推进 export job 状态。
