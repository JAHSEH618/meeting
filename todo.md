...
- [x] Phase K：移除会议安全分级（contracts securityLevel 枚举、meetings.security_level 列、Java SecurityLevel + 阻断门、meeting-web UI）+ TOS SDK 迁移 + 测试清理 —— 见 `RELEASE-NOTES-v1.1.0.md`。
- [x] 对齐审计（本次）：清掉 Phase K 残留 —— 任务消息 schema 仍 required securityLevel、Java payload 硬编码、`llm_call_logs.security_level NOT NULL` 运行时 bug（新迁移 V202606120830 drop 余下 4 列 + 枚举类型）、6 个 IT INSERT、死错误码 SECURITY_LEVEL_BLOCKED 链、ProdProfileValidator allow-confidential、prompt 模板安全等级行、ai-worker-web 整套安全等级 UI、meeting-web 残留断言/tsc 错误、Prometheus 死告警、smoke 脚本 payload、README/CLAUDE/structure/SPEC 文档漂移 —— 见 `docs/superpowers/2026-06-12-alignment-audit-report.md`。

## 代码评审修复（2026-06-12）

> 全库四端 code review（commit `9143010`）：14 Critical + 42 Important。报告见 `docs/superpowers/2026-06-12-code-review-report.md`；修复计划四卷 + 索引见 `docs/superpowers/plans/2026-06-12-review-remediation-index.md`（跨端协议决策与上线顺序约束以索引为准）。执行顺序：P1 ∥ P2 → P3 ∥ P4；**P2 的 120s 租约 TTL 不得先于 P1 心跳上线**。

### P1 `apps/ai-worker`（计划 p1-ai-worker）

- [x] C1：生产引擎补齐 8 步合同——`ALIGNMENT` 与 `RAG_INDEXING` 按 SPEC 降级矩阵跳进 `skippedSteps`（已查实 RAG_INDEXING 无法在 worker 侧实现：chunk 由 Java 建表、仅经 TEXT_EMBEDDING/RAG_REINDEX 内联下发），会议管线常态以 `PARTIAL_SUCCEEDED` 收尾；端到端集成测试用真实 Java 8 步消息 × 默认引擎跑通。
- [x] C2：每个运行中步骤每 20s 真心跳（progress 单调不减），心跳失败只告警不杀任务，asyncio 任务 finally 取消。
- [x] C3：pika 消费回调不再阻塞——流水线挪到工作线程，主线程 `process_data_events`，ack 走 `add_callback_threadsafe`。
- [x] C4：顶层异常安全网——非 WorkerPipelineError 也先发 `/fail`（注册 `WORKER_INTERNAL_ERROR`）再 reject；无音轨 ffprobe 映射 `AUDIO_CORRUPTED`。
- [x] I5–I12：rerank 全量打分后再取 topN；入站 HMAC 校验补 query string；dev 默认密钥在非 dev 环境 fail-closed（启动 + `/internal/ready`）；CUDA OOM 走 `ASR_GPU_OOM` + `report_oom_and_exit()`；补发 `/artifacts` 回调 + 真实 manifest id；错误码与 `error-codes.yaml` 对齐（contracts 同步提交）；admin BFF 上游异常 → 502 envelope；`time.sleep` → `asyncio.sleep`。

### P2 `apps/meeting-api`（计划 p2-meeting-api）

- [x] C1+C3：`WorkerPhaseCompletedListener` 异步化（专用线程池）+ 监听器/编排器所有读写包进 `TenantScopedTransaction` + WORKER_DAG_DONE 卡死恢复扫描器；非 superuser 角色的 RLS 集成测试守住回归；**对 worker 的 `PARTIAL_SUCCEEDED` + skippedSteps（ALIGNMENT/RAG_INDEXING）照常驱动 SUMMARY/EXTRACTION**。
- [x] C2：LLM/embed/rerank 调用移出 DB 事务（照 `ExportRenderService` 三段式），覆盖 RAG query、minutes、extraction。
- [x] C4+C5：租约模型重做——创建即 QUEUED 不预占租约（删 `worker_dev_001`），worker 首个回调认领；`completeWorkerPhase` 清租约；过期扫描只看 `WORKER_DAG_RUNNING`；wire `requeueOrphaned`（重置 phase、attempt+1、重发 MQ）+ `confirmCancelled`；`retry()` 重发消息；心跳续期 now+120s；callback 路径行级锁。
- [x] I6–I15：nonce 去重表 + 心跳 progress 单调守卫（只挡进度回退、**租约仍续期**）；~~`expectedInputVersion` 校验~~；~~`/artifacts` 真实持久化 + 完整校验链~~；embeddings 回调补租约/归属校验；~~任务终态回写 `meetings.status`~~；~~公共写接口幂等键强制 + 重放~~；~~RAG 缓存键带版本 + 权限后查 + STALE 失效~~；outbox 同聚合 fencing；~~回调错误码按 SPEC §7 表映射 + envelope 补 requestId/traceId~~。（**部分完成**: I6/I7/I10/I14 已实施，I8/I9/I11/I12/I15 需要重构/schema变更，已记录在 P2_DATA_INTEGRITY_SUMMARY.md）
...
- [x] I5–I8：commit/建会后失效对应 query；导出轮询超时显式报错 + unmount 清理；说话人决策按 label 全局锁定；transcriptVersion 缺失给显式提示。（完成于 2026-06-15）
- [x] I9+I10：分块流式 SHA-256（移植 meeting-web `sha256-stream.ts`）+ 空 ETag 快速失败；移除预填凭据。（完成于 2026-06-15）
