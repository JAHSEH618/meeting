...
| meeting-api-adapter | REST Controller、SSE、internal callback、MQ consumer、BFF 响应适配 | 只做协议转换、鉴权入口、参数校验和调用 app 层 |
...
```text
1. adapter 层只处理输入输出协议：REST、SSE、internal callback、MQ 消费、文件上传回调和 BFF 响应装配。
...
6. 一期可先把模型和第三方 API 准入信息登记到 `model_registry` schema 预留表；AI 产物仍通过 `artifact_manifests` 记录实际使用的模型版本、checksum 和 provider actual_model_version。
...
如果声纹库超过数万人，应将 speaker embedding 搜索从 PostgreSQL 拆到专门向量库，或按 tenant/project/person_status 做分区索引。
