...
> [!CAUTION]
> 绝对不要在生产环境使用阿里云主账号（Root AccessKey）。必须在阿里云 **RAM (访问控制)** 控制台中，为 Java 应用和远程 Worker 分别创建独立的 RAM 子用户，并授予最小必要权限。

下面是针对阿里云 RAM 控制台的**一步一步傻瓜式点击操作指南**：

---

#### 步骤一：创建自定义权限策略 (Policies)
我们需要先在控制台中录入我们的权限 JSON 模板，定义好“读写”与“只读”两条策略。

1. 登录 [阿里云控制台](https://home.console.aliyun.com/)。
2. 在顶部搜索框中输入 **`RAM`** 或 **`访问控制`**，点击搜索结果进入 **访问控制 (RAM) 控制台**。
3. 在左侧导航栏中，依次展开 **`权限管理` -> `权限策略`**。
4. 在页面右上方，点击蓝色按钮 **`创建权限策略`**。
5. 在创建策略页面，选择 **`脚本编辑`** 标签页。
6. 清空文本框中默认生成的 JSON 模板，并分别粘贴以下对应的 JSON 代码：

##### 策略 A：Java 读写权限策略 (`AliyunOSSMeetingJavaPolicy`)
适用于 Java 服务，允许对三个业务 Bucket 进行上传、读取、删除和获取元数据。
