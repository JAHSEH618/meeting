...
设计目标：
- 高信息密度 + 清晰的视觉层级
- 浅色 / 深色双主题（浅色为默认，深色为一等主题）+ 明亮的技术蓝，深色下营造精密仪器感
- 精确的排版和间距节奏
- 强对比度，确保可读性和操作效率
...
下面给出的是**深色主题**（`:root[data-theme="dark"]`）的核心 token；浅色主题是默认的 `:root` token 集（近白中性底 + 同一套语义色），主题切换机制见 [Theming（主题）](#theming主题)。
...
- **背景保持中性**：浅色主题用近白的中性底，深色主题为纯黑系；品牌个性通过明亮的蓝色主色传达
...
浅色是默认主题：`:root` 定义浅色 token 集，深色通过 `:root[data-theme="dark"]` 整组覆盖（两个前端共享的 `apps/*/src/shared/styles/tokens.css`，仅覆盖随主题变化的 token）。

- **偏好三态**：`light | dark | system`，持久化在两个 SPA 共享的 localStorage key `meeting.theme` 下（选择 `system` 时移除该 key）；同源部署使运维在两个控制台间保持同一选择。
- **无闪烁**：两个 `index.html` 都内置 pre-paint 内联脚本，在首帧前把解析结果写到 `<html data-theme="light|dark">`，深色用户不会看到浅色闪屏。
- **跟随系统**：偏好为 `system` 时跟随 `prefers-color-scheme`，OS 切换实时生效。
- **跨标签页同步**：监听 `storage` 事件，另一个同源标签页切换主题后本页即时同步。
- **切换控件**：`ThemeToggle` 单击循环 跟随系统 ◐ → 浅色 ☀ → 深色 ☾。
